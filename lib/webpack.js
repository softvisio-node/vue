...
export default class WebpackRunner {
    ...
    async #buildWebpackConfig () {
        if ( !this.#webpackConfig ) {
            var webpackConfigs = await import( new URL( "webpack.config.js", url.pathToFileURL( this.context + "/" ) ) );

            webpackConfigs = webpackConfigs.default;

            if ( webpackConfigs instanceof WebpackConfig ) {
                webpackConfigs = [webpackConfigs];
            }
            else {
                webpackConfigs = Object.values( webpackConfigs );
            }

            this.#validateEnv( webpackConfigs );
            ...
            // generate configs
            this.#webpackConfig = webpackConfigs.map( webpackConfig => webpackConfig.generate( options ) );
    ...
    #validateEnv ( configs ) {
        ...
        for ( const config of configs ) {
            if ( !config.schemas.length ) return;
