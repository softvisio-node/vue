...
export default class WebpackRunner {
    ...
    async #buildWebpackConfig () {
        if ( !this.#webpackConfigs ) {
            const configs = await import( new URL( "webpack.config.js", url.pathToFileURL( this.context + "/" ) ) );
            ...
            preprocessorParams.isProduction = env.isProduciotn;
            preprocessorParams.isDevelopment = env.isDevelopment;
            preprocessorParams.isCordova = this.#cordova;
            ...
            // build config
            for ( const entry of webpackConfigs ) {
    ...
    #validateConfigEnv ( config ) {
        ...
        process.stdout.write( `Validating environment for the "${config.name}" entry ... ` );
