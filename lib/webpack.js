...
export default class WebpackRunner {
    ...
    // properties
    get buildTag () {
        this.#buildTag ??= [this.#command, this.mode, this.#cordova ? "cordova" : null].filter( tag => tag ).join( "." );
    ...
    get buildTargets () {
        return `${ansi.ok( ` ${this.mode.toUpperCase()} ` )}${this.#cordova ? `, ${ansi.ok( ` CORDOVA ` )}` : ""}`;
    }
    ...
    async #getProcessEnv () {
        if ( !this.#processEnv ) {
            ...
            this.#processEnv = JSON.stringify( this.#processEnv );
        }
    ...
    #getProcessConfig () {
        if ( !this.#processConfig ) {
            ...
            this.#processConfig = JSON.stringify( this.#processConfig );
        }
