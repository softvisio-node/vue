...
export default class WebpackRunner {
    ...
    async #buildWebpackConfig () {
        if ( !this.#webpackConfig ) {
            const webpackConfig = ( await import( new URL( "webpack.config.js", url.pathToFileURL( this.context + "/" ) ) ) ).default;

            this.#validateEnv( webpackConfig );
            ...
            for ( const entry of webpackConfig ) {
                const config = entry.generate( options );
                ...
                this.#webpackConfig.push( config );
            }
        }
    ...
    #validateEnv ( config ) {
        ...
        for ( const entry of config ) {
            process.stdout.write( `Validating environment for the "${entry.name}" entry ... ` );
    ...
    async #getWebpackEnv () {
        if ( !this.#processEnv ) {
            ...
            if ( process.env.GIT_STATUS ) {
                this.#processEnv.GIT_STATUS = JSON.parse( process.env.GIT_STATUS );
            }
            else {
                const git = new Git( this.context );

                const status = await git.getStatus( { "releases": false, "pushStatus": false } );

                if ( !status.ok ) {
                    console.error( `Unable to get git status: ${status}` );
                }
                else {
                    this.#processEnv.GIT_STATUS = JSON.stringify( status.data );
                }
            }
        }
