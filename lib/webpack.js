...
export default class WebpackRunner {
    ...
    constructor ( { mode, command, analyzer, cache, cordova, listen, port } = {} ) {
        ...
        if ( this.#command === "dev" ) {
            ...
            if ( listen ) DEV_SERVER_OPTIONS.host = listen;
            if ( port ) DEV_SERVER_OPTIONS.port = port;
        }
        else {
