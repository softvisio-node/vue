...
export default class extends Store {
    ...
    async signin ( credentials ) {
        credentials = { ...credentials, "device_guid": this.#getDeviceGuid() };
    ...
    async signup ( data ) {
        data = { ...data, "device_guid": this.#getDeviceGuid() };
    ...
    #getDeviceGuid () {
        var guid = window.localStorage.getItem( DEVICE_GUID_KEY );

        if ( !guid ) {
            guid = uuidv4();

            window.localStorage.setItem( DEVICE_GUID_KEY, guid );
        }
