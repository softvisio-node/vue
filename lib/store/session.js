...
export default class extends Store {
    ...
    async _authorize ( options ) {
        ...
        if ( options.oauthProvider ) {
            const res = await this.#oauth( options.oauthProvider );
    ...
    // private
    async #oauth ( oauthProvider ) {
        const oauthUrl = new URL( window.location.href );
        oauthUrl.pathname = "/api/oauth.html";
        oauthUrl.hash = "";
        oauthUrl.searchParams.set( "oauthProvider", oauthProvider );

        const popup = window.open( oauthUrl, null, "toolbar=no, menubar=no, width=600, height=700" );
        ...
        const params = await new Promise( resolve => {
            ...
            window.addEventListener(
                ...
                e => {
                    ...
                    resolve( new URLSearchParams( e.data ) );
                },
                { "once": true }
            );
        } );
