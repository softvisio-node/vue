...
export default class extends Store {
    ...
    constructor () {
        ...
        firebase.on( "refresh", token => this.#syncPushNotificationsToken( token ) );
    }
    ...
    async #enablePushNotifications () {
        ...
        const tokenStored = await this.#syncPushNotificationsToken( token );
    ...
    #getPushNotificationsStatus () {
        ...
        if ( this.isAuthenticated ) {
            ...
            enabled = window.localStorage.getItem( PUSH_NOTIFICATIONS_ENABLED_KEY );
    ...
    async #syncPushNotificationsToken ( token ) {
        const tokenId = token.substring( 0, 16 ),
            storedTokenId = window.localStorage.getItem( PUSH_NOTIFICATIONS_TOKEN_ID_KEY );

        if ( tokenId === storedTokenId ) return true;

        const res = await this.$api.call( "", token );
