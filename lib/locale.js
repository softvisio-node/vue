...
if ( process.env.APP_LOCALES ) {
    ...
    for ( const locale of locales ) {
        ...
        if ( !LOCALES[locale] ) {
            LOCALES[locale] = { "name": `${locales[language].name}${country ? ` (${country.toUpperCase()})` : ""}]` };
...
if ( !LOCALES.en ) LOCALES.en = { "name": languages.en.name };
...
var locale = new URLSearchParams( window.location.search ).get( NAME ) || window.localStorage.getItem( NAME );

if ( !LOCALES[locale] ) locale = process.env.APP_DEFAULT_LOCALE;

if ( !LOCALES[locale] ) locale = "en";

locale = new Locale( null, { locale } );
