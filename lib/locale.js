...
if ( config.app.locales ) {
    const locales = config.app.locales.split( "," ).map( locale => locale.trim() );
...
// use default locale
if ( !LOCALES[localeId] ) {
    if ( config.app.defaultLocale && LOCALES[config.app.defaultLocale] ) {
        localeId = config.app.defaultLocale;
    }
...
const locale = new Locale( { "id": localeId, "currency": config.app.defaultCurrency || DEFAULT_CURRENCY } );
