...
export default class WebpackRunner {
    ...
    async #buildWebpackConfig () {
        if ( !this.#webpackConfig ) {
            ...
            var webpackConfig = await import( new URL( "webpack.config.js", url.pathToFileURL( this.context + "/" ) ) );

            webpackConfig = webpackConfig.default;

            if ( "name" in webpackConfig ) {
                webpackConfig = [webpackConfig];
            }
            else {
                webpackConfig = Object.values( webpackConfig );
            }
    ...
    #validateEnv ( configs ) {
        for ( const config of configs ) {
            ...
            process.stdout.write( `Validating environment for "${config.name}" config ... ` );

            this.#env.env = this.#getWebpackEnv();
