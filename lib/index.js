...
import localePlugin from "#lib/plugins/locale";
...
export default class App extends Events {
    ...
    #user = {};
    #permissions = new Set();
    #reAuthorizationMutex = new Mutex();
    ...
    async init () {
        ...
        if ( config.app.apiUrl ) {
            this.#api = Api.new( config.app.apiUrl, {
                ...
                "onReAuthorization": this.#onReAuthorization.bind( this ),
            } );
    ...
    async #onReAuthorization () {
        if ( !this.#reAuthorizationMutex.tryDown() ) return await this.#reAuthorizationMutex.signal.wait();

        const res = await this._onReAuthorization();

        this.#reAuthorizationMutex.signal.broadcast( res );
        this.#reAuthorizationMutex.up();
